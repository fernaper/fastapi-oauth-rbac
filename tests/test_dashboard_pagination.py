...
async def test_dashboard_pagination_and_filtering():
    ...
    async with AsyncSessionLocal() as db:
        ...
        with patch('fastapi_oauth_rbac.dashboard.router.RBACManager') as MockRBAC:
            rbac_mock = AsyncMock()
            ...
            with patch('fastapi_oauth_rbac.dashboard.router.templates.TemplateResponse') as MockTemplate:
                # 1. Test Default Pagination (Page 0, PageSize 10)
                await dashboard_index(mock_request, db, current_user, pageSize=10, page=0)
                ...
                # 2. Test Page 2
                await dashboard_index(mock_request, db, current_user, pageSize=10, page=2)
                args, kwargs = MockTemplate.call_args
                context = args[1]
                ...
                await dashboard_index(mock_request, db, current_user, filter='user1')
                args, kwargs = MockTemplate.call_args
                context = args[1]
                assert context['filtered_users'] == 11
                
                # 4. Test Filter by Status (active -> i % 2 == 0)
                await dashboard_index(mock_request, db, current_user, filter='active')
                args, kwargs = MockTemplate.call_args
                context = args[1]
                ...
                await dashboard_index(mock_request, db, current_user, filter='verified')
                args, kwargs = MockTemplate.call_args
                context = args[1]
                ...
                await dashboard_index(mock_request, db, current_user, filter='admin')
                args, kwargs = MockTemplate.call_args
                context = args[1]
