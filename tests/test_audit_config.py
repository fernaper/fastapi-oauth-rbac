...
async def test_audit_configuration(enabled: bool):
    ...
    async with engine.begin() as conn:
        if enabled:
            ...
            tables = [
                t for n, t in Base.metadata.tables.items() if n != 'audit_logs'
            ]
            await conn.run_sync(Base.metadata.create_all, tables=tables)

    async with engine.connect() as conn:
        res = await conn.run_sync(
            lambda sync_conn: engine.dialect.has_table(sync_conn, 'audit_logs')
        )
        print(f"Table 'audit_logs' exists: {res}")
        if enabled != res:
            print(
                f'ERROR: Expected audit_logs table existence to be {enabled}'
            )
        else:
